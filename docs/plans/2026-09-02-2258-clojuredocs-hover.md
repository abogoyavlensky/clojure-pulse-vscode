...
**Status: completed** (2026-09-03). Summary at the end.
...
- [x] **Step 1: Rewrite the renderer tests**
...
> Deviation (Task 2): `renderClojureDocsHtml` and `escapeHtml` stay until Task 4, because `src/clojureDocsPanel.ts` still imports them and removing them earlier would break the build between tasks; Task 4 deletes both with the panel. Var names are markdown-escaped in the header and in see-also link text (`clojure.core/*` would otherwise open emphasis).
...
> Deviation (Task 4): the HTML renderer and `escapeHtml` were removed here together with the panel (see the Task 2 note). `make check` first failed on stale `out/test/clojureDocsPanel.test.js` left behind by tsc after the source was deleted; `rm -rf out` before the check fixed it.
...
- [x] **Step 1: Rewrite the assertions**
...
**Implemented.** Show ClojureDocs now records a one-shot request (`src/clojureDocsRequest.ts`) and opens the editor hover focused via `editor.action.showHover` with `autoFocusImmediately`, after `editor.action.hideHover`. A `HoverProvider` (`src/clojureDocsHover.ts`, registered before the client starts so the server's part renders above it) answers only that request, looks the var up through `clojurePulse/clojureDocs`, and renders markdown (`buildClojureDocsMarkdown`): a header naming the var with "Available since" and the clojuredocs.org link, examples as ```` ```clojure ```` fences highlighted by the user's theme, and see-also links as trusted command URIs that re-run the command with the target var. The webview panel, its HTML renderer, and their tests are gone; the extension's `ExtensionApi` exposes the request holder for the end-to-end test. README and CHANGELOG describe the hover. Six commits, net fewer lines than the panel.

**Verification.** `make check` green (full suite: 7 request, 6 markdown, 6 provider tests among them); `npm run package` produces a 720 KB vsix with `data/clojuredocs.json`; the gated end-to-end test passes against `target/debug/clj-pulse`: our part appears in the real hover pipeline for `map` with a highlighted example, `str/join` resolves to `clojure.string/join` through the alias, the ordinary hover stays silent, and the real command's request reaches the provider.

**Issues encountered.** Stale compiled test files in `out/` after deleting the panel test made the first `make check` fail until `out/` was cleared. One codex run crashed while executing the VS Code suite concurrently with mine; rerun sequentially it passed. The Task 4 review flagged the then-stale end-to-end test and docs, both already scheduled as Tasks 5 and 6 and confirmed fixed by their reviews.

**Deviations, gathered.**
- Task 2: the HTML renderer stayed until Task 4 so the build never broke between tasks; var names are markdown-escaped in the header and link text.
- Task 4: renderer removed with the panel; `rm -rf out` before the check.

**What the plan could have specified better.** Deleting a source file leaves its compiled twin in `out/`; a plan that removes files should say to clear `out/` before the check. Otherwise it held up.
